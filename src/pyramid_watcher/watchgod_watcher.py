...
Copied from Samuel Colvin's watchgod.watchers
...
class AllWatcher:
    ...
    def _walk(self, dir_path, changes, new_files):
        for entry in os.scandir(dir_path):
            if entry.is_dir():
                ...
                if not old_mtime:
                    changes.add((FileChangeInfo.added, entry.path))
                elif old_mtime != mtime:
                    changes.add((FileChangeInfo.modified, entry.path))
    ...
    def check(self):
        ...
        if deleted:
            changes |= {(FileChangeInfo.deleted, entry) for entry in deleted}
